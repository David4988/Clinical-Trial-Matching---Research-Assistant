...
```bash
cd backend
.venv/Scripts/python -m pytest tests -q     # 719 tests, no network by default (add a local Postgres for the DB-backed ones; a local Ollama for the one live model test)
